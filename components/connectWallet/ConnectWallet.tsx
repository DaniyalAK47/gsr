...
interface AutoConnectLocalStorage {
  connectionKind: Exclude<ConnectionKind, "gnosisSafe">;
...
const SUPPORTED_WALLETS: Exclude<ConnectionKind, "gnosisSafe">[] = [
  "injected",
  "walletConnect",
  "walletLink",
  "portis",
      ...
      >
        <Flex sx={{ alignItems: "center" }}>
          ...
          >
            ...
            >
              <Icon
                sx={{ color: "black", position: "relative", top: "3px" }}
                name="arrow_right"
              />
            </Box>
          </Flex>
...
export function getInjectedWalletKind(): InjectedWalletKind {
  ...
  if (!w.web3 || typeof w.web3.currentProvider === "undefined")
    return "nonexistent";
  ...
  if (w.web3.currentProvider.constructor.name === "EthereumProvider")
    return "mist";

  if (w.web3.currentProvider.constructor.name === "Web3FrameProvider")
    return "parity";

  if (
    w.web3.currentProvider.host &&
    w.web3.currentProvider.host.indexOf("infura") !== -1
  )
    return "infura";

  if (
    w.web3.currentProvider.host &&
    w.web3.currentProvider.host.indexOf("localhost") !== -1
  )
    return "localhost";
...
type WalletKind =
  | Exclude<ConnectionKind, "injected" | "gnosisSafe">
  | InjectedWalletKind;
...
export function getConnectionDetails(walletKind: WalletKind): ConnectionDetail {
  ...
  return {
    ...{ userIcon: "someWallet_user" },
    ...connectionDetails[walletKind],
  };
}
...
export function getWalletKind(
  ...
  return connectionKind === "injected"
  ...
  return (
    ...
    >
      ...
      {web3Context.status === "error" && (
        <Alert
          variant="error"
          sx={{ fontWeight: "normal", borderRadius: "large" }}
        >
      ...
      <Grid columns={1} sx={{ maxWidth: "280px", width: "100%", mx: "auto" }}>
        ...
        {SUPPORTED_WALLETS.map((connectionKind) => {
          ...
          const { friendlyName, connectIcon } =
            getConnectionDetails(walletKind);
          const descriptionTranslation = isConnecting
            ? "connect-confirm"
            : "connect-with";
        ...
        <Box sx={{ mt: 4 }}>
          ...
          >
            <Text
              variant="paragraph2"
              sx={{ color: "inherit", fontWeight: "semiBold" }}
            >
