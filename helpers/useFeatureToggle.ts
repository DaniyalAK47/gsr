...
export type Feature = "TestFeature" | "AnotherTestFeature" | "Torus";

const configuredFeatures: Record<Feature, boolean> = {
  TestFeature: false, // used in unit tests
  AnotherTestFeature: false, // used in unit tests
  Torus: true,
  // your feature here....
};

export function configureLocalStorageForTests(
  data: { [feature in Feature]?: boolean }
) {
...
export function loadFeatureToggles(
  ...
  if (typeof localStorage !== "undefined") {
    ...
    const featureFlagsInLocalStorage =
      localStorage.getItem(FT_LOCAL_STORAGE_KEY);
    if (!featureFlagsInLocalStorage) {
      localStorage.setItem(
        FT_LOCAL_STORAGE_KEY,
        JSON.stringify(featuresSourcedFromCode)
      );
    } else {
...
export function getFeatureToggle(feature: Feature): boolean {
  if (typeof localStorage !== "undefined") {
    ...
    return (
      JSON.parse(userEnabledFeatures || "{}")[feature] ||
      configuredFeatures[feature]
    );
  }
  return false;
}
